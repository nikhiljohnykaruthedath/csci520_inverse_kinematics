...
void FK::computeLocalAndGlobalTransforms(
    ...
    for (int i = 0; i < jointUpdateOrder.size(); i++) {
        Mat3d RMMatrix, ROMatrix, localJointTransform;

        // Calculate RM and RO matrices
        double initRmMatrix[9] = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
        euler2Rotation(eulerAngles[jointUpdateOrder[i]], initRmMatrix, rotateOrders[jointUpdateOrder[i]]);
        RMMatrix = Mat3d(initRmMatrix);

        double initRoMatrix[9] = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
        euler2Rotation(jointOrientationEulerAngles[jointUpdateOrder[i]], initRoMatrix, RotateOrder::XYZ);
        ROMatrix = Mat3d(initRoMatrix);
        
        // Calculate local joint transforms
        localJointTransform = ROMatrix * RMMatrix;
        localTransforms[jointUpdateOrder[i]] = RigidTransform4d(localJointTransform, translations[jointUpdateOrder[i]]);
        ...
        int jointOrder = jointUpdateOrder[i];
        
        // If joint is root
        if (jointParents[jointOrder] == -1) {
            globalTransforms[jointOrder] = localTransforms[jointOrder];
        }
        // For other joints
        else {
            globalTransforms[jointOrder] = globalTransforms[jointParents[jointOrder]] * localTransforms[jointOrder];
        }
    }
}
